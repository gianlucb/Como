...
{
    [EventSource(Name = "Como.Cluster.EventManager")]
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        [Event(9999, Level = EventLevel.Error, Message = "{0}")]
        public void Error(string message)
        {
            if (this.IsEnabled())
            {
                WriteEvent(9999, message);
            }
        }

        [Event(9998, Level = EventLevel.Warning, Message = "{0}")]
        ...
        [Event(9997, Level = EventLevel.Informational, Message = "{0}")]
        public void NewEventCreated(string eventID, string uri)
        {
            if (this.IsEnabled())
            {
                WriteEvent(9997, "New Event created in the cluster: " + eventID + ". Listening at: " + uri);
            }
        }
